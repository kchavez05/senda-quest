import { 
  ...
  TAG_ROLL_D20, 
  TAG_ENEMIES_PREFIX, 
  TAG_PLAYER_HP_PREFIX, 
  TAG_PLAYER_MANA_PREFIX 
} from '../constants';

export function generateGMPrompt(action: string, isRoll: boolean, gameState: any, summary?: string): string {
  const contextLogs = gameState.logs.slice(-15).map((l: any) => `${l.sender.toUpperCase()}: ${l.text}`).join('\n');

  let historySection = '';
  if (summary) {
    historySection += `Past Story Summary:\n${summary}\n\n`;
  }
...
- If Combat Status is INACTIVE and an encounter begins:
  1. Use ${TAG_COMBAT_START} and ${TAG_ROLL_D20} immediately.
  ...
  4. List the initial enemies using the ${TAG_ENEMIES_PREFIX}Name1, Name2] tag.

- If Combat Status is ACTIVE:
  1. DO NOT use ${TAG_COMBAT_START}.
...
General Rules:
- When enemies are defeated or new ones appear, ALWAYS update the list using ${TAG_ENEMIES_PREFIX}Name1, Name2].
- If all enemies are defeated, use ${TAG_COMBAT_END}.
...
- If the player describes a custom action (e.g., "I try to swing from the chandelier" or "I attempt to reason with the guard"):
  ...
  2. If a roll is required, use ${TAG_ROLL_D20} and specify the attribute (e.g., "Roll for Dexterity to swing from the chandelier").
...
- If the player attempts to escape, talk their way out, or de-escalate AFTER you have prompted for initiative but BEFORE they have rolled it:
  ...
  2. Prompt for a Charisma (Diplomacy) roll using ${TAG_ROLL_D20}.
  3. If the roll is successful: Describe the de-escalation and use ${TAG_COMBAT_END}.
  4. If the roll fails: Describe the failure (e.g., the enemy is too enraged) and EXPLICITLY state that this roll result will be used as their Initiative. Proceed with the combat narrative immediately using that roll value. DO NOT use ${TAG_ROLL_D20} again and DO NOT ask for an initiative roll; the Diplomacy roll has already served that purpose.

Hidden Tags:
- ${TAG_COMBAT_START} to enter combat mode
- ${TAG_COMBAT_END} to exit combat mode
- ${TAG_ROLL_D20} to prompt the player for a d20 roll
- ${TAG_ENEMIES_PREFIX}Name1, Name2] to define/update the current enemies in combat
- ${TAG_PLAYER_HP_PREFIX}value] to update the player's current health. YOU MUST USE THIS whenever the player takes damage or is healed. ALWAYS include the specific damage or healing numbers (e.g., "The blade bites deep for 6 damage") directly in your narrative response.
- ${TAG_PLAYER_MANA_PREFIX}value] to update the player's current mana. YOU MUST USE THIS whenever the player gains or loses mana.`;
}
